...
"""
CAPS Permission Gate for Flux Theme
...
def has_capability(user=None, capability=None):
    """Check if user has a specific Flux Theme capability via CAPS."""
    if not user:
    ...
    if not capability.startswith("FT_"):
        capability = f"FT_{capability}"
